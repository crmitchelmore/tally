...
- Minimal authenticated flow implemented in `MainActivity` + `TallyViewModel`: challenges list → detail (entries) → create challenge dialog → add entry dialog → delete entry.
