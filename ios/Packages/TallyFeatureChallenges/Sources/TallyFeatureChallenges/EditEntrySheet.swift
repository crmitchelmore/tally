...
/// Sheet for editing an existing entry
public struct EditEntrySheet: View {
    ...
    public init(
        ...
        _sets = State(initialValue: entry.sets ?? [entry.count])
        _note = State(initialValue: entry.note ?? "")
        _selectedFeeling = State(initialValue: entry.feeling)
        
        // Parse date from ISO string
        let parsedDate = Self.dateFormatter.date(from: entry.date) ?? Date()
    ...
    public var body: some View {
        NavigationStack {
            ...
            .accessibilityIdentifier("editEntrySheet")
        }
    }
    ...
    private func setButton(_ title: String, index: Int, increment: Int, enabled: Bool) -> some View {
        Button {
            ...
            Text(title)
                ...
                .fontWeight(.medium)
                .frame(maxWidth: .infinity)
                .frame(minHeight: TallyMetrics.minTouchTarget)
