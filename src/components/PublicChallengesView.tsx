...
export function PublicChallengesView({ userId, onBack }: PublicChallengesViewProps) {
  ...
  const buildPublicChallenges = (): PublicChallenge[] => {
  ...
  const publicChallenges = buildPublicChallenges()
  
  const filteredChallenges = publicChallenges.filter(challenge => 
    challenge.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    challenge.username.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const getDaysRemaining = (year: number) => {
    const now = new Date()
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredChallenges.map((challenge, index) => {
                ...
                const daysRemaining = getDaysRemaining(challenge.year)
                const progressPercent = Math.min(challenge.progress, 100)
                        ...
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="w-10 h-10">
                              <AvatarImage src={challenge.avatarUrl} alt={challenge.username} />
                              <AvatarFallback>{challenge.username.slice(0, 2).toUpperCase()}</AvatarFallback>
                            </Avatar>
                            <div>
                              <p className="font-semibold text-sm">{challenge.username}</p>
                              {isOwnChallenge && (
