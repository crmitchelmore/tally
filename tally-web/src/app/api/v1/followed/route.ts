...
export async function GET() {
  try {
    ...
    const follows = await getFollowsByUser(authResult.userId);

    // Get challenge details for each follow
    const followedChallengesPromises = follows.map(async (f) => {
      const challenge = await getChallengeById(f.challengeId);
      if (!challenge || !challenge.isPublic) return null;

      // Get total reps
      const entries = await getEntriesByChallenge(challenge.id);
      ...
      // Get follower count
      const followerCount = await getFollowerCount(challenge.id);

      // Get owner info
      const owner = await getUserByClerkId(challenge.userId);
    ...
    const followedChallenges = (await Promise.all(followedChallengesPromises)).filter(Boolean);
