...
import type { Challenge, Entry, HeatmapDay, SetData, FeelingType } from "@/types";
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6 mt-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="p-4 border-2 border-border">
            ...
            <div className="text-2xl font-bold font-mono">{stats.bestDay?.count.toLocaleString() || 0}</div>
            {stats.bestDay && (
        ...
        <Card className="p-6 border-2 border-border">
          <h2 className="text-lg font-semibold mb-4 uppercase tracking-wider text-sm text-muted-foreground">
            {challenge.timeframeUnit === "month"
              ? "Monthly"
              : challenge.timeframeUnit === "custom"
                ? "Custom Period"
                : "Yearly"}{" "}
            Activity
          </h2>
          <HeatmapCalendar data={heatmapData} year={challenge.year} size="large" onDayClick={handleDayClick} />
        </Card>
          ...
          <Card className="p-6 border-2 border-border">
            ...
            <div className="space-y-3">
              {recentEntries.map((entry) => {
                const feelingOption = entry.feeling
                  ? FEELING_OPTIONS.find((o) => o.type === entry.feeling)
                  : null;
                ...
                return (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {entry.note && <div className="text-sm text-muted-foreground mt-1">{entry.note}</div>}
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className="text-base font-bold font-mono">
                        {entry.count}
                      </Badge>
                      <Button
                        variant="ghost"
