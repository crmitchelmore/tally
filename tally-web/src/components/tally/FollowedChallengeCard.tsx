...
export function FollowedChallengeCard({
  challenge,
  entries,
  ownerName,
  ownerAvatarUrl,
  onUnfollow,
}: FollowedChallengeCardProps) {
  const username = ownerName || `User ${challenge.userId.slice(0, 6)}`;
  ...
  const totalDays = challenge.endDate
    ? Math.ceil(
        (new Date(challenge.endDate).getTime() - new Date(challenge.startDate || new Date()).getTime()) /
          (1000 * 60 * 60 * 24)
      )
    : 365;
      ...
      <Card className="overflow-hidden border-2 border-dashed">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            ...
            <Button onClick={onUnfollow} variant="ghost" size="sm" className="h-8 px-2">
